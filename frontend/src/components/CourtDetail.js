 ...
 * Layout: thông tin sân (trái) | timeline giờ + form đặt (phải)
 */
export default function CourtDetail({
  ...
  useEffect(() => {
    ...
    if (idx >= 0) setSelectionRange({ start: idx, end: Math.min(hours.length - 1, idx + Math.max(1, duration) - 1) });
  ...
  const getSlotStatus = (hour) => {
    ...
    const booking = bookingRequests.find(
      r => String(r.courtId) === String(selectedCourt.id) &&
           r.date === selectedDate && r.hour === hour
    );
  ...
  const isDisabled = (hour) => {
    ...
    return s === 'past' || s === 'approved' || s === 'done';
  };
  ...
  const handleBook = () => {
    ...
    setPhoneError('');
    onGoToPayment({ selectedCourt, selectedDate, selectedHour, duration, totalPrice, depositAmount: deposit,
  ...
  const slotStatusMeta = {
    available: { label: 'Trống',     bg: '#d1fae5', color: '#065f46', border: '#6ee7b7', cursor: true  },
    approved:  { label: 'Đã đặt',   bg: '#fee2e2', color: '#991b1b', border: '#fca5a5', cursor: false },
    pending:   { label: 'Chờ duyệt',bg: '#fef3c7', color: '#92400e', border: '#fcd34d', cursor: false },
    done:      { label: 'Kết thúc', bg: '#f3f4f6', color: '#6b7280', border: '#d1d5db', cursor: false },
    past:      { label: '—',        bg: '#f9fafb', color: '#d1d5db', border: '#e5e7eb', cursor: false },
  };
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 16px 64px', fontFamily: 'inherit' }}>
      ...
      <button
        ...
        style={{ background: 'none', border: 'none', color: '#4361ee', fontWeight: 700,
          fontSize: '0.92rem', cursor: 'pointer', marginBottom: 20, padding: 0, display: 'flex', alignItems: 'center', gap: 6 }}
      >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
        ...
        {isVip && (
          <span style={{ background: 'linear-gradient(135deg,#f59e0b,#d97706)', color: '#fff',
            padding: '3px 14px', borderRadius: 20, fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.05em' }}>
            ✦ VIP
          </span>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 28, alignItems: 'start' }}>
        ...
        <div>
          {/* Gallery */}
          <div style={{ borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,.1)', marginBottom: 20, position: 'relative' }}>
          ...
          {/* Giá */}
          <div style={{ background: '#f8faff', borderRadius: 14, padding: '16px 18px', marginBottom: 16,
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.88rem', color: '#475569', marginBottom: 6 }}>
              <span>Giờ thường (05:00–17:00)</span>
              <strong style={{ color: '#1e293b' }}>{selectedCourt.price.toLocaleString()} VNĐ/giờ</strong>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.88rem', color: '#475569', marginBottom: 6 }}>
              <span>Giờ tối (17:00–22:00)</span>
              <strong style={{ color: '#dc2626' }}>{Math.floor(selectedCourt.price*1.3).toLocaleString()} VNĐ/giờ</strong>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.88rem', color: '#475569' }}>
              <span>Đặt cọc</span>
              <strong style={{ color: '#4361ee' }}>50% khi đặt</strong>
            </div>
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 14, padding: '16px 18px', border: '1.5px solid #f1f5f9' }}>
            ...
            {[
              ['🏸','Thảm Yonex cao cấp'],
              ['💡','Ánh sáng thi đấu chuẩn'],
              ['🛡️','Sàn chống trơn trượt'],
              ['🕒','Mở cửa 05:00 – 22:00'],
              ['💧','Nước uống & WC sạch'],
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              {selectedDate && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    <label style={labelStyle}>Chọn giờ <span style={{ fontWeight:400, color:'#94a3b8', fontSize:'0.78rem' }}>(kéo chuột để chọn nhiều giờ)</span></label>
                  </div>

                  {/* Legend nhỏ */}
                  <div style={{ display:'flex', gap:10, marginBottom:10, flexWrap:'wrap' }}>
                    {[['available','Trống'],['approved','Đã đặt'],['pending','Chờ duyệt']].map(([k,t]) => (
                      <div key={k} style={{ display:'flex', alignItems:'center', gap:5, fontSize:'0.75rem', color:'#64748b' }}>
                        <span style={{ width:10, height:10, borderRadius:'50%', background: slotStatusMeta[k].bg,
                          border:`1.5px solid ${slotStatusMeta[k].border}`, display:'inline-block' }} />
                        {t}
                      </div>
                    ))}
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <span style={{ fontSize: '0.65rem', fontWeight: 500, opacity: 0.85 }}>
                            {inSel ? '✓' : isEve ? '🌙' : meta.label === 'Trống' ? '☀️' : meta.label}
                          </span>
                        </button>
                      );
                    })}
              ...
              {selectedHour && (
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                    ...
                    <span style={{ fontWeight:800, fontSize:'1.1rem', color:'#1e293b', minWidth:50, textAlign:'center' }}>
                      {duration} giờ
                    </span>
              ...
              <button onClick={handleBook}
                style={{ width:'100%', background:'linear-gradient(135deg,#4361ee,#3a0ca3)',
                  ...
                  fontSize:'1rem', fontWeight:800, cursor:'pointer', letterSpacing:'0.02em',
      ...
      <style>{`
        @media (max-width: 760px) {
          .cd-two-col { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </div>
  );
