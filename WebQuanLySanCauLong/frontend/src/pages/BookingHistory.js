...
const getStatus = (req) => {
  if (req.status === "pending") {
    return { label: "Chờ duyệt sân", icon: "⏳", bg: "#fff7ed", color: "#c2410c", border: "#fed7aa" };
  }
  if (req.status === "approved") {
    if (req.paymentStatus === "pending") {
      return { label: "Chờ thanh toán", icon: "💳", bg: "#fff7ed", color: "#d97706", border: "#fed7aa" };
    }
    if (req.paymentStatus === "deposit_sent") {
      return { label: "Chờ duyệt tiền", icon: "⏳", bg: "#f0fdf4", color: "#059669", border: "#bbf7d0" };
    }
    if (req.paymentStatus === "paid" || req.paymentStatus === "remaining_paid") {
      return { label: "Đã thuê thành công", icon: "✅", bg: "#f0fdf4", color: "#15803d", border: "#bbf7d0" };
    }
    return { label: "Đã duyệt sân", icon: "✅", bg: "#f0fdf4", color: "#15803d", border: "#bbf7d0" };
  }
  if (req.status === "rejected") {
    return { label: "Đã từ chối", icon: "❌", bg: "#fef2f2", color: "#b91c1c", border: "#fecaca" };
  }
  if (req.status === "done") {
    return { label: "Hoàn thành", icon: "🏆", bg: "#f5f3ff", color: "#6d28d9", border: "#ddd6fe" };
  }
                  ...
                  <div style={{
                    ...
                    <div style={{ gridColumn: "1/-1", display: "flex", gap: 10, marginTop: 8, flexWrap: "wrap" }}>
                      {req.status === "approved" && req.paymentStatus === "pending" && (
                        <button onClick={() => payForBooking && payForBooking(req)} style={{
